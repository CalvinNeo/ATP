...
atp_callback_arguments ATPSocket::make_atp_callback_arguments(ATP_CALLBACKTYPE_ENUM method, OutgoingPacket * out_pkt, const ATPAddrHandle & addr){
    if(out_pkt == nullptr){
...
OutgoingPacket * ATPSocket::basic_send_packet(uint16_t flags){
    ...
    ATPPacket pkt = ATPPacket{
        (uint16_t)(seq_nr & seq_nr_mask), // seq_nr, updated in send_packet
        (uint16_t)(ack_nr & seq_nr_mask), // ack_nr
        (uint16_t)peer_sock_id, // peer_sock_id
        0,// opts_count
        (uint8_t)flags, // flags
        (uint16_t)my_window // my window
    };
    OutgoingPacket * out_pkt = new OutgoingPacket{
        ...
        0, // selective_acked
        0, // ahead_handled
        0, // need_resend, update by `send_packet`/`check_unsend_packet`
...
void ATPSocket::clear_state(){
    ...
    cur_window_packets = window_packets_unlimited; 
    used_window_packets = 0; 
    enable_cork = false;

    cur_window = window_packets_unlimited;
    used_window = 0;
    peer_window = window_packets_unlimited;
    my_window = window_packets_unlimited;

    current_mss = ATP_MSS_CEILING;

    reorder_count = 0;

    #ifdef USE_OLD_SACK_FIELD
    peer_max_sack_count = 0;
    my_max_sack_count = 5;
    #else
    peer_max_sack_count = 0;
    my_max_sack_count = 4;
    ...
    // Options
    reuse_port_flag = false;
    on_listen_port = false;
...
int ATPSocket::init_fork(ATPSocket * origin){
    switch_state(CS_IDLE);
    sockfd = origin->sockfd;
    ...
    reuse_port_flag = true;

    #if defined (ATP_LOG_AT_DEBUG) && defined(ATP_LOG_UDP)
        log_debug(this, "UDP Socket forked from %s, sockfd %d.", origin->to_string(), sockfd);
    #endif
    return sockfd;
}
...
ATP_PROC_RESULT ATPSocket::connect(const ATPAddrHandle & to_addr){
    ...
    context->register_to_look_up(this);
    ...
    switch_state(CS_SYN_SENT);
    return result;
}


ATP_PROC_RESULT ATPSocket::listen(uint16_t host_port){
    switch_state(CS_LISTEN);
    // set port
    get_local_addr().set_port(host_port);
    // register this port to context's listen list
    if (context->register_listen_port(this, host_port) == ATP_PROC_OK)
    {
        this->on_listen_port = true;
        #if defined (ATP_LOG_AT_DEBUG)
        log_debug(this, "I am listening on port %u.", host_port);
        #endif
    }
}
...
ATPSocket * ATPSocket::fork_basic(){
...
ATP_PROC_RESULT ATPSocket::accept(const ATPAddrHandle & to_addr, OutgoingPacket * recv_pkt){
    ...
    {
        // This is the first SYN request
        // if(re_listen) {
        //     goto REUSED_SOCKET;
        // }
    }else if(reuse_port_flag) {
        // Port reusing is allowed, so this socket will not be removed from listening queue.
REUSED_SOCKET:
        re_listen = false;
        char * _opt_sockid_field = recv_pkt->find_option(ATP_OPT_SOCKID);
        if(_opt_sockid_field == nullptr) return ATP_PROC_OK; // This is a bad packet. Ignore it!
        uint16_t _peer_sock_id = *reinterpret_cast<uint16_t*>(_opt_sockid_field + 2 * sizeof(uint8_t));
        if (this->peer_sock_id == _peer_sock_id)
        {
            ...
            // But ATP don't need that because ATP can have very short TIME_WAIT time.
            // We treat it as a new connection to another socket listening at the same port.
            atp_callback_arguments arg = make_atp_callback_arguments(ATP_CALL_BEFORE_REP_ACCEPT, nullptr, to_addr);
            result = invoke_callback(ATP_CALL_BEFORE_REP_ACCEPT, &arg);
            if (result == ATP_PROC_REJECT)
            {
                return ATP_PROC_OK;
            }else {
                // atp_callback_arguments::socket is `newsock` rather than `this`
                arg = make_atp_callback_arguments(ATP_CALL_ON_FORK, nullptr, to_addr);
                arg.socket = this;
                // After this call, arg.socket will be set to a newly created socket.
                result = invoke_callback(ATP_CALL_ON_FORK, &arg);
                if (result != ATP_PROC_OK || arg.socket == this)
                {
                    #if defined (ATP_LOG_AT_DEBUG)
                    log_debug(this, "You must create a new socket.");
                    #endif
                    return ATP_PROC_ERROR;
                }else{
                    ATPSocket * newsock = arg.socket;
                    newsock->accept(to_addr, recv_pkt);
                    return ATP_PROC_OK;
                }
            }
        }
    } else {
        // In this case, corresponding packets won't be dispatched to socket.
        return ATP_PROC_OK;
    }
    ...
    if (result == ATP_PROC_OK)
    {
        // accept this SYN
        dest_addr = to_addr;
        // Now our socket can be located
        if(!reuse_port_flag){
            // If we don't need to reuse port, we will remove socket from listening
            on_listen_port = false;
            context->deregister_listen_port(get_local_addr().host_port());
        }
        context->register_to_look_up(this);
    #if defined (ATP_LOG_AT_NOTE) 
        print_out(this, recv_pkt, "rcv");
    #endif
        init_connection(recv_pkt, false);
        ...
        result = send_packet(out_pkt);
        #if defined (ATP_LOG_AT_DEBUG)
        log_debug(this, "Reject SYN request from %s by sending RST.", ATPSocket::make_hash_code(peer_sock_id, dest_addr));
        #endif
    }else{
        #if defined (ATP_LOG_AT_DEBUG)
...
ATP_PROC_RESULT ATPSocket::receive(OutgoingPacket * recv_pkt, size_t real_payload_offset){
    if (recv_pkt->get_head()->get_fin())
    {
        ...
        size_t actual_received = invoke_callback(ATP_CALL_ON_RECV, &arg);
        // TODO FIXME IMPORTANT handle situation when this is not drained read
        if (actual_received < arg.length)
        {
            #if defined (ATP_LOG_AT_DEBUG)
            log_debug(this, "User read only %u of %u bytes.", actual_received, arg.length);
            #endif
        }else{
            #if defined (ATP_LOG_AT_DEBUG)
            log_debug(this, "User read %u bytes(drained).", actual_received);
            #endif
            return ATP_PROC_OK;
        }
    }
...
void ATPSocket::check_unsend_packet(){
    ...
    for(OutgoingPacket * out_pkt : outbuf){
        ...
        if (out_pkt && (out_pkt->transmissions == 0 || out_pkt->need_resend))
        {
...
ATP_PROC_RESULT ATPSocket::send_packet(OutgoingPacket * out_pkt, bool flush_packets, bool adhoc){
    ...
    if(adhoc){
        ...
        if (out_pkt->is_empty_ack())
        {
            ...
            // ACK packet is ad-hoc sent packet, don't enqueue to `outbuf`(always delete at once(except SYN, FIN))
            result = send_packet_noguard(out_pkt);
            delete out_pkt;
            out_pkt = nullptr;
            // Cancel schedule_ack
            ...
            // Enqueue `out_pkt` for proper time to send and for potential resend
            PUSH_OUTBUF(out_pkt);
            if(flush_packets){
                check_unsend_packet();
            }
...
ATP_PROC_RESULT ATPSocket::close(){
    ...
    switch(conn_state){
        ...
        case CS_SYN_RECV:
            #if defined (ATP_LOG_AT_DEBUG)
                log_debug(this, "Close: Close a socket which is not connected yet.");
            #endif
            result = ATP_PROC_ERROR;
            break;
        case CS_LISTEN:
        case CS_PASSIVE_LISTEN:
        {
            // Call `close` on a listening socket is valid operation
            switch_state(CS_DESTROY);
            result = ATP_PROC_OK;
            break;
        }
        case CS_CONNECTED:
...
bool ATPSocket::writable() const{
    ...
    // A writable socket can accept inputs
    switch(conn_state){
        case CS_UNINITIALIZED:
        case CS_IDLE:
        case CS_LISTEN:
...
bool ATPSocket::readable() const{
    // A readable socket is capable of reading data from peer
    switch(conn_state){
        case CS_UNINITIALIZED:
        case CS_IDLE:
        case CS_LISTEN:
...
bool ATPSocket::eof() const{
    // When a socket is not readable after connection is established, eof = true
    switch(conn_state){
        case CS_UNINITIALIZED:
        case CS_IDLE:
        case CS_LISTEN:
...
ATP_PROC_RESULT ATPSocket::write(const void * buf, const size_t len){
    ...
    // Flush packets when all packets are created
    check_unsend_packet();
    ...
    if (action == ATP_PROC_OK)
    {
        ...
        switch(conn_state){
            ...
            case CS_LAST_ACK: // B
            {
                ...
                #if defined (ATP_LOG_AT_DEBUG)
                    log_debug(this, "Recv the last ACK for my FIN from Peer, Both side on the connection die.");
                #endif
                break;
            }
            case CS_TIME_WAIT: 
...
ATP_PROC_RESULT ATPSocket::handle_recv_packet(OutgoingPacket * recv_pkt, bool from_cache){
    ...
    else if (action == ATP_PROC_OK)
    {
        ...
        if(!recv_pkt->get_head()->get_fin()){
            ...
            // SYN packet will be handled in `handle_recv_packet_hard`, but FIN packet will be handled here
...
OutgoingPacket * ATPSocket::construct_packet_from_buffer(const char * buffer, size_t len){
    // This function construct a new OutgoingPacket from buffer.
    // Must **copy** from "kernel", rather than operate in-place, because buffer is limited
...
void ATPSocket::init_connection(OutgoingPacket * recv_pkt, bool active){
    ATP_PROC_RESULT result = ATP_PROC_OK;
    if (active)
    {
        switch_state(CS_CONNECTED);
    }else{
        switch_state(CS_SYN_RECV);
    }
    ...
    if (active)
    {
        ...
        invoke_callback(ATP_CALL_ON_ESTABLISHED, &arg);
...
ATP_PROC_RESULT ATPSocket::process(const ATPAddrHandle & addr, const char * buffer, size_t len){
    OutgoingPacket * recv_pkt = construct_packet_from_buffer(buffer, len);
    const ATPPacket * pkt = recv_pkt->get_head();
    ...
    if(pkt->get_syn() && pkt->get_ack()){
        ...
        // Connection established on side A, the active open part
    ...
    if (result == ATP_PROC_OK)
    {
        // Check if there is any packet which can be acked, after `recv_pkt` is acked
        while(!inbuf.empty()){
            OutgoingPacket * top_packet = inbuf[0];
            result = handle_recv_packet(top_packet, true);
            if(result == ATP_PROC_DROP)
            {
                // Whether should I delete packet? No. Or will cause double free
                // // delete top_packet;
                result = ATP_PROC_OK;
                continue;
            }
    ...
    if (result == ATP_PROC_FINISH)
    {
        ...
        {
            // Destroy this socket immediately
            this->destroy();
        }
    } 
    return result;
...
ATP_PROC_RESULT ATPSocket::do_selective_ack_packet(char * peer_sack_data, uint8_t peer_sack_data_size){
    ...
    for(size_t i = 0; i < count; i++){
        ...
        if (pkt_iter != outbuf.end())
        {
            ...
            #if defined (ATP_LOG_AT_DEBUG)
                fprintf(stdout, "[%c]%u(%lu) ", op_sgn, cur_pkt->full_seq_nr, ack);
                fprintf(stderr, "[%c]%u(%lu) ", op_sgn, cur_pkt->full_seq_nr, ack);
            #endif
        }else{
            #if defined (ATP_LOG_AT_DEBUG)
                op_sgn = 'N';
            #endif
            #if defined (ATP_LOG_AT_DEBUG)
                fprintf(stdout, "[%c](%lu) ", op_sgn, ack);
                fprintf(stderr, "[%c(%lu) ", op_sgn, ack);
            #endif
        }
    }
    #if defined (ATP_LOG_AT_DEBUG)
        ...
        fprintf(stderr, "\n");
    #endif
    // Remove SACKed packet
    #if !defined(_ATP_NEW_BUFFER)
    SWITCHTO_MARKED(outbuf);
    #if defined (ATP_LOG_AT_DEBUG)
    ...
    SWITCHTO_FULLSEQ(outbuf);
    #else
    for (auto iter = outbuf.begin(); iter != outbuf.end(); iter++){
        if ((*iter) && (*iter)->marked)
        {
            delete (*iter);
            (*iter) = nullptr;
...
ATP_PROC_RESULT ATPSocket::do_ack_packet(OutgoingPacket * recv_pkt){
    // `ack == n` means peer's packet [..n] are all acked
    ...
    while(!outbuf.empty()){
        OutgoingPacket * out_pkt = outbuf.front(); 
        #if defined(_ATP_NEW_BUFFER)
            if(!out_pkt) continue;
        #endif
        ...
        {
            // Ack all outgoing packet whose `seq_nr` is less than `my_seq_acked_by_peer`
            #if defined (ATP_LOG_AT_DEBUG)
            ...
            if (out_pkt->selective_acked)
            {
                ...
                if(out_pkt->transmissions > 0 && out_pkt->is_promised_packet() && !out_pkt->get_head()->get_urg()){
                    // Making sure `transmissions > 0` is very important, because due to delayed ACK, 
                    ...
                    // Meanwhile, it will not be calculated in `used_window`.
                    #if defined (ATP_LOG_AT_DEBUG)
            ...
            // This packet is not Acked by peer yet, or it's SACKed by peer.
            break;
        }
    }
}
...
void ATPSocket::update_rto(OutgoingPacket * recv_pkt){
    ...
    {
        // We can only compute RTT of not re-transmited packet.
        // This strategy is same as TCP's strategy.
...
void ATPSocket::destroy_hard(){
    // Wait 2MSL to destroy
    #if defined (ATP_LOG_AT_DEBUG)
        log_debug(this, "Log socket to destroy list.");
    #endif
...
void ATPSocket::destroy(){
    if(reuse_port_flag && on_listen_port){
        // Go back to listen state, reuse this socket because it listens port
        #if defined (ATP_LOG_AT_DEBUG)
            log_debug(this, "Reset socket to listening mode.");
        #endif
        // A listening socket must maintain these two callbacks
        #ifdef _ATP_PERFORM_CLEAN_SWITCH
        atp_callback_func * old_before = callbacks[ATP_CALL_BEFORE_REP_ACCEPT];
        atp_callback_func * old_fork = callbacks[ATP_CALL_ON_FORK];
        clear_state();
        reuse_port_flag = true;
        on_listen_port = true;
        #endif

        switch_state(CS_PASSIVE_LISTEN);
        re_listen = true;

        #ifdef _ATP_PERFORM_CLEAN_SWITCH
        callbacks[ATP_CALL_BEFORE_REP_ACCEPT] = old_before;
        callbacks[ATP_CALL_ON_FORK] = old_fork;
        #endif
    }else{
        destroy_hard();
    }
...
ATP_PROC_RESULT ATPSocket::check_timeout(){
    ...
    if (delay_ack_timeout != 0 && (current_ms > delay_ack_timeout))
    {
    ...
    if (!outbuf.empty())
    {
        ...
        if (rto_timeout != 0 && (current_ms > rto_timeout))
        {
            ...
            #endif
            {
                ...
                for(OutgoingPacket * out_pkt : outbuf){
                    if (out_pkt && out_pkt->is_promised_packet() && !out_pkt->selective_acked)
                    {
    ...
    if (persist_timeout != 0 && (current_ms > persist_timeout))
    {
...
void ATPSocket::switch_state(CONN_STATE_ENUM new_state){
